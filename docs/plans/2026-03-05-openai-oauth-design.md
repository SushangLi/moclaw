...
Add OAuth login option for OpenAI ChatGPT Plus/Pro subscribers in MoClaw, allowing them to use OpenClaw without a separate API key.
...
- MoClaw currently only supports API key input for OpenAI
...
MoClaw delegates token management to OpenClaw CLI:
