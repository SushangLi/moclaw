...
**Architecture:** Hybrid approach — MoClaw opens Electron BrowserWindow for OAuth login, delegates token management to OpenClaw CLI (`openclaw models auth login --provider openai-codex`). UI adds auth method toggle in OpenAI provider tab.
