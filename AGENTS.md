...
MoClaw는 OpenClaw AI 에이전트를 원클릭으로 설치하는 **Electron 데스크톱 인스톨러**. electron-vite + React + Tailwind CSS 4 기반이며 macOS/Windows를 지원한다.
...
소스코드와 바이너리 모두 `SushangLi/moclaw` 단일 저장소에서 관리.
...
- macOS: `https://github.com/SushangLi/moclaw/releases/latest/download/moclaw.dmg`
- Windows: `https://github.com/SushangLi/moclaw/releases/latest/download/moclaw-setup.exe`

**빌드 파일명**: `electron-builder.yml`에서 버전 없이 고정 (`moclaw.dmg`, `moclaw-setup.exe`)

### Vercel 배포 (docs/ + api/)

- `docs/`: 정적 마케팅 페이지 (moclaw.kr)
...
This section documents the customizations made to the MoClaw project for MomoAI provider integration.

## Changes Made

### 1. Provider Type Changes

**Files modified:**

- `src/renderer/src/constants/providers.ts` - Changed Provider type from `'anthropic' | 'google' | 'openai' | 'minimax' | 'glm'` to `'momoai'` only
- `src/renderer/src/App.tsx` - Changed default provider to `'momoai'`
- `src/renderer/src/steps/ConfigStep.tsx` - Updated type definitions
- `src/preload/index.ts` - Updated IPC types (2 places)
- `src/preload/index.d.ts` - Updated type declarations (2 places)
- `src/main/ipc-handlers.ts` - Updated handler types (2 places)
- `src/main/services/onboarder.ts` - Updated OnboardConfig interface

### 2. MomoAI Provider Configuration

**`src/renderer/src/constants/providers.ts`:**
...
```typescript
{
  ...
  pattern: /^momo_/,
  models: [
    { id: 'momo_222', name: '花满楼的Qwen3.5 Plus', desc: 'Qwen 3.5 Plus', price: '0.4 CNY / M tokens' },
    { id: 'momo_223', name: '花满楼的Deepseek V3.2', desc: 'Deepseek V3.2', price: '2.0 CNY / M tokens' },
    { id: 'momo_221', name: '花满楼的GLM-5', desc: 'GLM-5', price: '3.5 CNY / M tokens' },
    { id: 'momo_220', name: '花满楼的Minimax M2.5', desc: 'Minimax M2.5', price: '2.0 CNY / M tokens' },
    { id: 'momo_219', name: '花满楼的kimi-k2.5', desc: 'Kimi K2.5', price: '5.0 CNY / M tokens' }
  ]
}
```

### 3. Onboarder Configuration (MomoAI Integration)

**`src/main/services/onboarder.ts`:**

- Removed CLI auth flags for MomoAI (was causing `--momoai-api-key` error)
- Added config patching with MomoAI provider configuration:
  - baseUrl: `http://hub.momoai.pro/api/agent-proxy`
  - models array with name field (required by OpenClaw)
- Updated `DEFAULT_MODELS` to use `'momo_222'` as default

### 4. UI Changes

**`src/shared/i18n/locales/zh/steps.json`:**

- Changed title from "选择 AI 提供商" to "选择智能体"
- Changed desc from "选择要使用的 AI 提供商并获取 API 密钥" to "选择要使用的MOMO AI智能体并获取API密钥"
- Added "optional" translation key

**`src/renderer/src/steps/ConfigStep.tsx`:**

- Made Telegram bot token optional (removed required label)
- Simplified validation to only require API key

**`src/renderer/src/steps/ApiKeyGuideStep.tsx`:**

- Simplified to only show model selection (no provider tabs, no OAuth)

**`src/renderer/src/components/ProviderSwitchModal.tsx`:**

- Simplified to work with MomoAI only
- Removed provider tabs and OAuth flow
- Shows model selection + API key input

### 5. Bug Fixes

- Fixed `MODEL_SPECS` type error (removed minimax reference)
- Fixed `providerPlaceholders` object syntax error
- Fixed `ProviderSwitchModal` blank page issue (invalid provider data handling)
- Fixed config patching to include `name` field in model object (was causing "expected string, received undefined" error)

## MomoAI Models

| ID       | Name                  | Price              |
| -------- | --------------------- | ------------------ |
| momo_222 | 花满楼的Qwen3.5 Plus  | 0.4 CNY / M tokens |
| momo_223 | 花满楼的Deepseek V3.2 | 2.0 CNY / M tokens |
| momo_221 | 花满楼的GLM-5         | 3.5 CNY / M tokens |
| momo_220 | 花满楼的Minimax M2.5  | 2.0 CNY / M tokens |
| momo_219 | 花满楼的kimi-k2.5     | 5.0 CNY / M tokens |

## Base URL

MomoAI API endpoint: `http://hub.momoai.pro/api/agent-proxy`
